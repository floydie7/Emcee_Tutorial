...
xl = np.linspace(0.0, 2.0)
y_true = a_true*xl**2 + b_true*xl + c_true
x = np.sort(2*np.random.rand(N))
yerr = np.random.normal(0,0.27,N)
...
print("Maximum likelihood values of parameters are:\n a={0:.3f}, b={1:.3f}, and c={2:.3f} \n True values: a={3}, b={4}, c={5}".format(a_ml, b_ml, c_ml, a_true, b_true, c_true))
#%%
fig, ax = plt.subplots()
ax.plot(xl, y_true, color='r')
ax.errorbar(x, y, yerr=yerr, fmt='k.')
ax.plot(xl, a_ml * xl**2 + b_ml * xl + c_ml, 'k--',label='$\chi^2$ fit');
ax.legend(loc=2)
plt.show()
...
ndim, nwalkers = 3, 100     # Set the number of dimensions of parameter space and the nubmer of walkers to explore the space.
pos = [result['x'] + 1e-4 * np.random.randn(ndim) for i in range(nwalkers)]     # Set the inital position of the walkers in the space.

# Set up and run the sampler.
sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(x, y, yerr))
sampler.run_mcmc(pos, 250); # Run sampler at initial position pos for 500 steps.
#%%
'''
...
for a, b, c in samples[np.random.randint(len(samples), size=100)]:
    plt.plot(xl, a*xl**2+b*xl+c, color="k", alpha=0.1)
plt.plot(xl, a_true*xl**2+b_true*xl+c_true, color="r", lw=1, alpha=0.8)
